...
public class OperacoesNoBancoDeDados {
	...
	public OperacoesNoBancoDeDados() {
		...
		//O nosso atributo conexão vai receber um nova conexão com o Banco de Dados
	...
	public void Cadastrar(DadosAgenda objeto) {
		...
		try {
			...
			//values - Valores
			String informacoesSql =
					"insert into tabela_cadastro" +
					"(nome, telefone, email, endereco, nascimento, filhos)" +
					"values(?, ?, ?, ?, ?, ?)";
	...
	public void Alterar(DadosAgenda objeto) {
		...
		try {
			...
			//tabela_cadastro - tabela do banco de dados
			//where - onde
			String informacoesSql = "update tabela_cadastro set " +
	...
	public void Excluir(DadosAgenda objeto) {
		...
		try {
			...
			String informacoesSql = "delete from tabela_cadastro where id = ?";
				...
				//Preparando / Tratando a execu��o dos comando SQL
