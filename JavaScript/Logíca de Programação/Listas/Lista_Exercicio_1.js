...
Você está organizando um evento e precisa gerenciar uma lista de c
onvidados. 
...
- Ordenar Convidados: Antes de exibir a lista de convidados, 
certifique-se de que está ordenada alfabeticamente.*/
...
// A função recebe um parâmetro 'nome', que é uma string representando
// o nome do convidado.
function adicionarConvidado(nome) {
  // Verifica se o nome já existe na lista usando o método 'includes'.
  // O método 'includes' retorna true se o item estiver no array,
  // caso contrário, retorna false.
  if (!listaDeConvidados.includes(nome)) {
    // Adiciona o nome ao array usando o método 'push'.
    // 'push' adiciona o elemento ao final do array e retorna o
    // novo comprimento do array.
    listaDeConvidados.push(nome);

    // Ordena a lista de convidados alfabeticamente usando o
    // método 'sort'.
    // 'sort' altera o próprio array e também retorna a referência
    // para o array.
    listaDeConvidados.sort();
  } else {
    // Se o nome já estiver na lista, imprime uma mensagem
    // indicando que já está na lista.
...
function removerConvidado(nome) {
  // Utiliza o método 'indexOf' para encontrar o índice do nome no
  // array.
  // 'indexOf' retorna o índice do primeiro elemento encontrado que
  // satisfaz a condição ou -1 se nenhum for encontrado.
  let indice = listaDeConvidados.indexOf(nome);

  if (indice !== -1) {
    // Remove o elemento no índice encontrado usando 'splice'.
    // O primeiro argumento é o índice para começar a remover, o
    // segundo é o número de elementos a remover.
    listaDeConvidados.splice(indice, 1);
  } else {
    // Se o nome não for encontrado, imprime uma mensagem
...
function checarPresenca(nome) {
  ...
  if (listaDeConvidados.includes(nome)) {
    // Se estiver na lista, imprime uma mensagem confirmando sua
    // presença.
    console.log(nome + " está na lista de convidados.");
  } else {
    // Se não estiver na lista, imprime uma mensagem indicando que
    // não está na lista.
...
function listarConvidados() {
  ...
  // 'join' converte todos os elementos do array em strings, se
  // necessário, e os concatena.
...
function numeroDeConvidados() {
  // Utiliza a propriedade 'length' para obter o número de elementos
  // no array.
  console.log("Total de convidados:", listaDeConvidados.length);
}

// Função para alterar o nome de um convidado.
// Recebe dois parâmetros, 'nomeAtual' que é o nome atual do convidado
// na lista, e 'novoNome', que é o novo nome a ser registrado.
function alterarConvidado(nomeAtual, novoNome) {
  // Primeiro, utiliza o método 'indexOf' para encontrar o índice do
  // nome atual na lista de convidados.
  // Se o nome não existir na lista, 'indexOf' retorna -1.
  let indice = listaDeConvidados.indexOf(nomeAtual);

  if (indice !== -1) {
    // Verifica se o índice é diferente de -1, o que indica que o convidado foi encontrado.
    ...
    if (!listaDeConvidados.includes(novoNome)) {
      // Atribui o novo nome ao índice encontrado, atualizando
      // diretamente na lista de convidados.
      listaDeConvidados[indice] = novoNome;

      // Ordena novamente a lista de convidados após a alteração,
      ...
      // Caso o novo nome já exista na lista, imprime uma mensagem
      // de erro.
      console.log(
        novoNome +
          " já está na lista de convidados. Escolha um nome diferente.",
      );
    }
  } else {
    // Se o nome atual não foi encontrado na lista, imprime uma
    // mensagem indicando que não pode ser alterado.
...
// Novamente listando convidados e mostrando o número total para
