// Define uma lista inicial chamada 'lista' com três elementos do tipo
// string.
let lista = ["maça", "banana", "cereja"];

// Utiliza o método `push` para adicionar o elemento 'laranja' ao final da lista 'lista'.
// O método `push` modifica o array original, adicionando o novo
// elemento no final.
lista.push("laranja");
...
// Acessa o elemento no índice 1 do array 'lista' (que é 'banana') e
// substitui por 'uva'.
// Os índices de array em JavaScript começam em 0, então o índice 1
// refere-se ao segundo elemento.
lista[1] = "uva";

// Exibe no console a lista após a substituição de 'banana' por
// 'uva'.
console.log("Lista após mudar banana para uva:", lista);

// Utiliza o método `splice` para remover 1 elemento a partir do
// índice 2 do array 'lista'.
...
// Utiliza a propriedade `length` do array para obter o número de
// elementos no array 'lista'.
// A propriedade `length` retorna o tamanho total do array.
console.log("Tamanho da lista:", lista.length);

// Utiliza o método `includes` para verificar se o elemento 'maça'
// está presente no array 'lista'.
// O método `includes` retorna `true` se o elemento estiver presente,
...
// Utiliza o método `includes` para verificar se o elemento 'banana'
// está presente no array 'lista'.
// Como o elemento 'banana' foi substituído por 'uva' anteriormente, o
// retorno esperado é `false`.
...
Combinando splice() para inserção e remoção de itens em qualquer 
posição de um array.
*/
...
// O primeiro argumento '2' indica o índice onde o novo elemento deve
// ser inserido.
...
// Exibe no console a lista atualizada com o número 3 adicionado,
// resultando em [1, 2, 3, 4, 5].
console.log("Números com 3 adicionado:", numeros);

// Remove o número 2 do array utilizando novamente o método splice.
// O primeiro argumento '1' indica o índice do elemento a ser removido
// (número 2).
// O segundo argumento '1' indica que apenas um elemento será removido.
numeros.splice(1, 1);

// Exibe no console a lista após a remoção do número 2, resultando em
// [1, 3, 4, 5].
...
// Declara um array 'frutas' contendo três elementos: 'maça', 'banana',
//  e 'cereja'.
let frutas = ["maça", "banana", "cereja"];

// Utiliza o método forEach para percorrer cada elemento do array
// 'frutas'.
// A função dentro de forEach recebe dois parâmetros: 'item', que é o
// elemento atual do array, e 'index', que é o índice do elemento atual.
// A cada iteração, essa função é chamada, e ela imprime o índice e o
// item correspondente.
...
// Aplica o método 'filter' para extrair números pares do array
// 'numeros_2'.
// A função dentro de 'filter' utiliza uma expressão que retorna
// verdadeiro (true) para números pares.
// 'n % 2 === 0' verifica se o número n é divisível por 2 sem resto,
// característica de números pares.
...
// Utiliza o método 'map' para calcular o quadrado de cada número no
// array 'numeros_3'.
// A função passada a 'map' pega cada elemento 'n' e retorna seu
// quadrado 'n * n'.
...
// 'numeros_4'.
// A função de redução recebe um acumulador 'acc' e o valor atual
// 'cur', somando-os e passando o resultado para a próxima iteração.
...
/*
5. Procurar itens
Os métodos find() e findIndex() são usados para procurar no array o 
primeiro
 item que atende a uma condição especificada, retornando o item ou seu 
...
// A função passada a 'find' retorna verdadeiro para o primeiro número
// que é maior que 10.
...
/*
6. Ordenar itens
O método sort() organiza os itens de um array de acordo com a condição 
definida.
*/
...
// Sem argumentos, 'sort' ordena os elementos assumindo que são
...
// Declara um array 'frutas_3' idêntico ao 'frutas_2' para demonstrar
// a reversão.
let frutas_3 = ["banana", "maça", "cereja"];

// Aplica o método 'reverse' para inverter a ordem dos elementos do
// array 'frutas_3'.
frutas_3.reverse();
...
/*
8. Concatenar Arrays
Você pode usar o método concat() para unir dois ou mais arrays, 
...
// Utiliza o método 'concat' para combinar 'array1' e 'array2' em um
// novo array 'arrayConcatenado'.
// O método 'concat' não altera os arrays originais, mas retorna um
// novo array que é a união dos dois.
...
// Define um array 'idades' contendo valores inteiros representando
// idades.
let idades = [32, 33, 18, 40];

// Aplica o método 'every' para verificar se todas as idades no array
// 'idades' são de 18 anos ou mais.
// A função passada para 'every' retorna verdadeiro se a condição
// (idade >= 18) for verdadeira para todos os elementos.
let todosAdultos = idades.every((idade) => idade >= 18);

// Exibe no console se todos no grupo são adultos, baseado na condição
// de serem maiores de 18 anos.
console.log("Todos são adultos:", todosAdultos);

/*
10. Verificar se Algum Elemento Atende a uma Condição
O método some() testa se pelo menos um dos elementos no array passa 
...
// Utiliza o método 'some' para verificar se pelo menos uma idade no
// array 'idade_2' é de 18 anos ou mais.
// A função passada para 'some' retorna verdadeiro se a condição
// (idade_2 >= 18) for
// verdadeira para pelo menos um elemento.
let algumAdulto = idade_2.some((idade_2) => idade_2 >= 18);

// Exibe no console se há pelo menos um adulto no grupo, baseado na
// condição de ser maior de 18 anos.
...
novo array a partir de início até fim (fim não incluído). O array 
original não será modificado.
*/

// Inicializa um array 'frutas_4' contendo cinco tipos de frutas.
let frutas_4 = ["banana", "maça", "laranja", "uva", "cereja"];

// Utiliza o método 'slice' para extrair uma subseção do array
// 'frutas_4'.
...
Você pode converter strings em arrays usando o método split(), muito 
útil para processamento de texto.
*/

// Define uma string 'frase' contendo a saudação "Olá mundo!".
let frase = "Olá mundo!";

// Utiliza o método 'split' para dividir a string 'frase' em palavras,
// usando um espaço como delimitador.
// O resultado é um array 'palavras' onde cada elemento é uma palavra
// da frase original.
...
Conversamente, você pode juntar elementos de um array em uma única 
string usando o método join().
*/

// Define um array 'palavras_2' contendo as palavras "Olá" e "mundo!".
let palavras_2 = ["Olá", "mundo!"];

// Utiliza o método 'join' para concatenar os elementos do array
// 'palavras_2' em uma única string.
// O espaço ' ' é usado como separador entre os elementos no resultado
// final.
