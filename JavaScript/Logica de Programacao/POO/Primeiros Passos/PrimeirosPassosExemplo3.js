...
class Pessoa {
  // Construtor da classe é chamado automaticamente quando um novo
  // objeto desta classe é criado.
  constructor(nome) {
    // 'this.nome' é uma propriedade do objeto Pessoa. Aqui, 'this'
    // refere-se ao objeto que está sendo criado.
    // O valor passado para o parâmetro 'nome' no momento da criação
    // do objeto é atribuído à propriedade 'nome'.
    this.nome = nome; // Armazena o nome da pessoa como uma string
    // dentro do objeto.

    // 'this.acordado' é uma propriedade do objeto que indica se a
    // pessoa está acordada ou não.
    // Inicialmente, toda nova pessoa é criada como não acordada,
    // então 'this.acordado' é definido como 'false'.
    this.acordado = false; // Armazena um valor booleano (verdadeiro ou falso).

    // 'this.comendo' é uma propriedade do objeto que indica se a
    // pessoa está comendo ou não.
    // Assim como 'acordado', cada pessoa nova não está comendo
    // quando criada, então isso é definido como 'false'.
    this.comendo = false; // Armazena um valor booleano.

    // 'this.dirigindo' é uma propriedade do objeto que indica se a
    // pessoa está dirigindo ou não.
    // Inicialmente, presume-se que a pessoa não está dirigindo, então
    // 'this.dirigindo' é definido como 'false'.
    this.dirigindo = false; // Armazena um valor booleano.

    // Resumindo, o construtor inicializa um objeto Pessoa com as
    // propriedades nome, acordado, comendo e dirigindo.
    // Isso prepara o objeto para representar diferentes estados e
    // ações de uma pessoa na vida real.
  }

  // Método para acordar
  acordar() {
    // A condição 'if (!this.acordado)' verifica se a propriedade
    // 'acordado' do objeto é falsa (ou seja, se a pessoa não está
    // acordada).
    if (!this.acordado) {
      // Se a condição for verdadeira (a pessoa não está acordada),
      ...
      // Se a condição for falsa (a pessoa já está acordada), esta
  ...
  // Método para dormir
  dormir() {
    // Verifica se a pessoa está acordada e simultaneamente não está
    // comendo nem dirigindo.
    if (this.acordado && !this.comendo && !this.dirigindo) {
      // Se todas as condições forem verdadeiras, ou seja, a pessoa
      // está acordada e não está nem comendo nem dirigindo:
      ...
      console.log(
        `${this.nome} não pode dormir enquanto está comendo ou dirigindo.`,
      ); // Exibe no console que a pessoa não pode dormir enquanto realiza essas atividades.
    }
  }

  // Método para comer
  comer() {
    // Primeira condição: Verifica se a pessoa não está comendo, está
    // acordada, e não está dirigindo.
    // Cada parte da condição é avaliada para garantir que a ação de
    // comer só pode ser iniciada sob condições apropriadas.
    if (!this.comendo && this.acordado && !this.dirigindo) {
      // Se a pessoa não está comendo, está acordada e não está
      // dirigindo:
      ...
      // Terceira condição: Se nenhuma das condições acima for
      // verdadeira, a pessoa deve estar dormindo ou dirigindo.
  ...
  pararDeComer() {
    // Verifica se a propriedade 'comendo' do objeto é verdadeira, o que
    // indica que a pessoa está comendo.
    if (this.comendo) {
      // Se a pessoa está comendo:
      console.log(`${this.nome} parou de comer.`); // Exibe uma mensagem
      // no console informando que a pessoa parou de comer.
      this.comendo = false; // Altera o estado de 'comendo' para false, indicando que a pessoa não está mais comendo.
    } else {
      // Se a propriedade 'comendo' é falsa, ou seja, a pessoa não estava
      // comendo:
  ...
  // Método para dirigir
  dirigir() {
    // Verifica se a pessoa não está dirigindo, está acordada e não está
    // comendo simultaneamente.
    // Esta condição assegura que a pessoa só possa dirigir se estiver
    // ativa e não ocupada com outras atividades.
    if (!this.dirigindo && this.acordado && !this.comendo) {
      // Se a pessoa atende aos critérios para dirigir:
      console.log(`${this.nome} começou a dirigir.`); // Exibe no console
      // que a pessoa iniciou a condução.
  ...
  pararDeDirigir() {
    // Verifica se a propriedade 'dirigindo' do objeto é verdadeira, ou
    // seja, se a pessoa está atualmente dirigindo.
    if (this.dirigindo) {
      ...
      // Se a propriedade 'dirigindo' é falsa, significa que a pessoa não
...
// Testes de diferentes situações para demonstrar as capacidades do
// objeto 'pessoa'.
...
// Testes das condiçoes para demonstrar as capacidades do objeto
// 'pessoa'.
pessoa.acordar(); // Acorda a pessoa, fazendo com que o estado 'acordado' seja verdadeiro.
