...
Se você tentar adicionar um valor que já existe no conjunto, ele será 
ignorado.
...
// Inicia um novo conjunto vazio usando a estrutura de dados Set, que
// armazena elementos únicos.
let conjunto = new Set();

// Adiciona valores ao conjunto. O método 'add' insere cada novo
// elemento, garantindo que não haja duplicatas.
conjunto.add("A");
conjunto.add("B");
conjunto.add("C");
conjunto.add("D");

// Utiliza o método 'has' para verificar a presença de elementos
// específicos no conjunto.
...
// Limpa todos os elementos do conjunto usando o método 'clear', resulta
// ndo em um conjunto vazio.
conjunto.clear();
...
// Itera sobre os elementos do conjunto usando o método 'forEach'.
//  Este método executa uma função para cada elemento.
console.log("Elementos do conjunto:");
conjunto.forEach((valor) => console.log(valor));

// Converte o conjunto em um array usando o operador de espalhamento
// (spread operator).
// O operador de espalhamento '...' é usado para extrair todos os
// elementos do conjunto 'conjunto'.
let arrayConjunto = [...conjunto];

// Exibe no console o array resultante, mostrando os elementos que
// foram no conjunto.
console.log("Conjunto convertido para array:", arrayConjunto);

// Cria outro conjunto com alguns valores diferentes para demonstração
// de operações de conjunto.
// Inicializa 'outroConjunto' com três elementos: 'Y', 'Z', 'W'.
let outroConjunto = new Set(["Y", "Z", "W"]);

// Realiza a união de dois conjuntos, combinando elementos de ambos os
// conjuntos sem duplicatas, usando o operador spread.
// O operador '...' é usado para expandir ambos os conjuntos dentro de
// um novo conjunto, garantindo que apenas elementos únicos sejam
// mantidos.
...
// Realiza a interseção de dois conjuntos, encontrando elementos
// comuns aos dois conjuntos.
// Cria um novo conjunto 'intersecao' para armazenar os resultados da
// interseção.
...
conjunto.forEach((valor) => {
  ...
  if (outroConjunto.has(valor)) {
    // Se 'outroConjunto' contém o elemento, ele é adicionado ao
    // conjunto 'intersecao'.
...
// Inicializa um novo conjunto chamado 'diferenca' como uma cópia do
// conjunto original 'conjunto'.
// O novo conjunto é criado usando o operador de espalhamento
// (spread operator) para incluir todos os elementos do 'conjunto'.
let diferenca = new Set(conjunto);

// Itera sobre cada elemento do 'outroConjunto' para determinar se
// algum desses elementos também está presente no conjunto 'diferenca'.
outroConjunto.forEach((valor) => {
  // Verifica se o conjunto 'diferenca' contém o elemento atual.
  // O método 'has' é usado para checar a presença de um elemento no
  // conjunto.
  if (diferenca.has(valor)) {
    // Se o elemento está presente, ele é removido do conjunto
    // 'diferenca'.
    // O método 'delete' é usado para remover o elemento
    // especificado do conjunto.
    diferenca.delete(valor);
  }
});

// Após concluir a iteração e a remoção de elementos comuns, o
// conjunto 'diferenca' contém apenas os elementos que estão no
// 'conjunto' original, mas não no  'outroConjunto'.
// Exibe no console os elementos restantes no conjunto 'diferenca',
// representando a diferença entre os dois conjuntos.
