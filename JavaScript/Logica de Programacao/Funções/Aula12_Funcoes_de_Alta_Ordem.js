...
filtro decide se um elemento do array deve ser incluído no resultado 
final.
*/

// Define uma função de alta ordem chamada 'filtrarArray'.
// Esta função é projetada para filtrar elementos de um array baseado
// em um critério definido por uma função de filtro passada como
// argumento.
// Recebe dois argumentos: um array 'array' e uma função
// 'funcaoFiltro' que determina se um elemento deve ser incluído no
// array filtrado.
function filtrarArray(array, funcaoFiltro) {
  // Cria um array vazio 'arrayFiltrado' para armazenar os elementos
  // que passam pelo filtro.
  let arrayFiltrado = [];

  // Utiliza um loop 'for...of' para iterar sobre cada elemento do
  // array fornecido.
  for (let elemento of array) {
    // Aplica a função de filtro 'funcaoFiltro' a cada elemento do
    // array.
    ...
    if (funcaoFiltro(elemento)) {
      // Adiciona o elemento ao array 'arrayFiltrado' caso ele passe
      // pelo filtro.
      arrayFiltrado.push(elemento);
    }
  }

  // Retorna o array 'arrayFiltrado' contendo todos os elementos que
...
// Recebe um único argumento 'numero' e retorna verdadeiro se
// 'numero' for par, caso contrário, retorna falso.
...
// Recebe um único argumento 'numero' e retorna verdadeiro se
// 'numero' for maior que 10, caso contrário, retorna falso.
function maiorQueDez(numero) {
  return numero > 10;
}

// Demonstra o uso da função de alta ordem 'filtrarArray' com
// diferentes funções de filtro.

// Cria um array 'numeros' contendo uma série de valores numéricos
// para serem filtrados.
let numeros = [1, 2, 3, 4, 5, 11, 12, 13, 14];

// Filtra o array 'numeros' usando a função 'ePar' como critério de
// filtro.
// Armazena o resultado no array 'numerosPares', que conterá apenas
// os números pares de 'numeros'.
let numerosPares = filtrarArray(numeros, ePar);

// Filtra o array 'numeros' usando a função 'maiorQueDez' como
// critério de filtro.
// Armazena o resultado no array 'numerosMaioresQueDez', que conterá
// apenas os números maiores que 10 de 'numeros'.
...
// Imprime o array 'numerosMaioresQueDez' no console, mostrando os
