...
(function () {
  ...
  // Esta função incrementa o valor de 'contador' por um e então imprime o novo valor
  // no console.
  ...
  function resetar() {
    ...
    console.log("Contador resetado."); // Imprime uma mensagem indicando o reset do
    // contador.
  }

  // Simula o uso das funções 'incrementar' e 'resetar' dentro da IIFE.
  // Como essas chamadas estão dentro da IIFE, elas são executadas imediatamente após
  // a definição.
...
que ilustra uma aplicação prática diferente. Neste caso, vamos usar uma IIFE para
 implementar um módulo simples que faz o gerenciamento de tarefas. Este módulo 
 permitirá
...
const gerenciadorDeTarefas = (function () {
  ...
  // Esta variável armazena as tarefas adicionadas e é acessível apenas dentro da
  // IIFE, mantendo o estado das tarefas privado.
  let tarefas = [];

  // Define a função 'adicionar', responsável por adicionar uma nova tarefa ao array
  // 'tarefas'.
  // A função recebe um argumento 'tarefa', que é uma string representando a tarefa
  // a ser adicionada.
  ...
  // Define a função 'listar', que imprime todas as tarefas armazenadas no array
  // 'tarefas'.
  function listar() {
  ...
  // A IIFE retorna um objeto contendo referências para as funções 'adicionar' e
  // 'listar'.
  // Esse objeto permite a interação com o gerenciador de tarefas de fora da IIFE,
  // mantendo as variáveis internas privadas.
  return {
    adicionar,
    listar,
  };
...
// Após adicionar algumas tarefas, chama o método 'listar' para exibir todas as
// tarefas armazenadas.
