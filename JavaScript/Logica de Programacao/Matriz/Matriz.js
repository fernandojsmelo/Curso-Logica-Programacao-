...
/*
  Esta matriz é composta por três linhas e três colunas, onde cada 
  elemento é acessado através de dois índices: um para a linha e outro 
  para a coluna.
...
// Acessa o elemento na linha 1, coluna 1 (lembrando que o índice
// começa em 0)
...
// A função 'forEach' é usada para iterar sobre cada elemento do array
// 'matriz'.
// Cada elemento aqui é um sub-array representando uma linha da matriz.
matriz.forEach((linha) => {
  // Para cada 'linha' (que é um sub-array da matriz), o método 'push'
  // é chamado.
  // O método 'push' adiciona um novo elemento ao final do array.
  // Aqui, estamos adicionando o número 13 ao final de cada linha da
  // matriz.
  linha.push(13);
});

// Após a execução do 'forEach', cada linha da matriz terá um novo
// elemento (13) adicionado ao seu final.
// Isso efetivamente adiciona uma nova coluna à matriz, onde cada
// elemento dessa nova coluna é 13.
...
// A função 'forEach' é utilizada para percorrer cada elemento do
// array 'matriz'.
// Aqui, cada elemento é um sub-array (referido como 'linha'), que
// representa uma linha da matriz.
matriz.forEach((linha) => {
  // Dentro do 'forEach', usamos o método 'join' no sub-array 'linha'
  // para converter todos os seus elementos em uma única string.
  ...
  // dados na matriz) em uma string única, com cada elemento separado
  // por espaço.
  let linhaFormatada = linha.join(" ");

  // O resultado da junção é então passado para 'console.log', que
...
// Inicializando a variável 'soma' para acumular a soma dos elementos
// da matriz 'soma' é inicializada com o valor 0 para começar a
...
for (let i = 0; i < matriz1.length; i++) {
  ...
  // 'j' é o índice do elemento dentro da linha, começando de 0 até o
  // número de elementos na linha (exclusivo).
  for (let j = 0; j < matriz1[i].length; j++) {
    // Soma os valores dos elementos.
    // 'matriz1[i][j]' acessa o elemento na i-ésima linha e j-ésima
    // coluna da matriz.
...
// Após os loops, a variável 'soma' contém a soma total de todos os
// elementos da matriz.
// Se desejar imprimir ou utilizar a soma, você pode adicionar um
// console.log aqui ou retornar a 'soma'.
...
Criar a matriz transposta de uma matriz 2x3 (converter linhas em 
colunas):
*/
...
// Criando uma matriz para a transposta. Note que as dimensões são
// invertidas: 3x2
// 'transposta' será a nova matriz onde as linhas e colunas da
// 'matriz2' são invertidas.
let transposta = [];

// Inicializando a matriz transposta com arrays vazios para cada
...
// Usa 'forEach' para percorrer cada linha da matriz 'transposta' e
// imprimir.
...
// Inicializando a variável 'max' com o menor valor possível para um
...
// O loop externo itera sobre as linhas da matriz. 'i' é o índice de
// cada linha.
for (let i = 0; i < matriz3.length; i++) {
  ...
  for (let j = 0; j < matriz3[i].length; j++) {
    // Atualizando 'max' se o elemento atual é maior que o valor atual
    // de 'max'. Compara o elemento atual da matriz, 'matriz3[i][j]',
    // com o valor atual armazenado em 'max'.
    if (matriz3[i][j] > max) {
...
// Após os loops completarem, a variável 'max' contém o maior valor
// encontrado na matriz.
// 'console.log' é usado para imprimir este valor, mostrando o maior
// elemento da matriz.
...
// Definindo a matriz 3x3
// 'matriz5' é definida como uma matriz quadrada com três linhas e
// três colunas.
// Cada sub-array representa uma linha da matriz.
let matriz5 = [
  [1, 2, 3], // Primeira linha da matriz
...
// Ela será usada para armazenar os elementos de 'matriz5' após a
// rotação.
let rotacionada = [
  [0, 0, 0],
  [0, 0, 0],
...
// Após completar a rotação, cada linha da matriz 'rotacionada' é
// impressa.
console.log("Matriz Rotacionada 90 graus no sentido horário:");

// O método 'forEach' é usado para iterar sobre cada linha da matriz
// 'rotacionada'.
...
// 'matriz6' é declarada como um novo array com três elementos. Cada
// elemento deste array representará uma linha da matriz.
...
// O método 'forEach' é usado para percorrer cada linha da matriz
// 'matriz6'.
matriz6.forEach((linha) => {
  // Cada 'linha' do array 'matriz6' é convertida em uma string e
  // impressa no console.
  // O método 'join' é utilizado para transformar os elementos do
  // array linha em uma string única, separada por espaços.
...
// 'matriz7' é uma matriz quadrada 3x3, onde cada sub-array representa
// uma linha da matriz.
let matriz7 = [
  [1, 2, 3], // Primeira linha da matriz
  [4, 5, 6], // Segunda linha da matriz
...
// 'diagonalPrincipal' será usado para coletar os elementos que estão
// na diagonal principal da matriz.
let diagonalPrincipal = [];

// Usando um laço for para extrair a diagonal principal
// O loop itera sobre as linhas da matriz. A variável 'i' serve como
// índice tanto para a linha quanto para a coluna correspondente na
// mesma iteração.
for (let i = 0; i < matriz7.length; i++) {
  // A diagonal principal de uma matriz quadrada contém os elementos
  // onde o índice da linha é igual ao índice da coluna.
  // Portanto, 'matriz7[i][i]' acessa o elemento na diagonal principal
  // na linha 'i' e coluna 'i'.
...
// 'C' e 'D' são matrizes de dimensão 2x2, onde cada sub-array
// representa uma linha da matriz.
let C = [
...
// 'somaMatrizes' é inicializada com valores zero em cada posição
// correspondente para preparar para a soma.
let somaMatrizes = [
...
for (let i = 0; i < C.length; i++) {
  ...
  for (let j = 0; j < C[i].length; j++) {
    ...
    // Soma os elementos correspondentes das matrizes C e D e armazena
    // o resultado na matriz 'somaMatrizes'.
    somaMatrizes[i][j] = C[i][j] + D[i][j];
  }
}

// Imprimindo a matriz resultante
// Após completar a soma, imprime a matriz 'somaMatrizes' linha por
// linha.
...
somaMatrizes.forEach((linha) => {
  // Cada 'linha' do array 'somaMatrizes' é convertida em uma string
  // com 'join', separando os elementos por espaço para facilitar a
  // visualização.
...
Uma matriz bidimensional é semelhante a uma tabela ou uma grade, com 
linhas e colunas. 

Vamos criar uma matriz 3x3 e demonstrar como acessar e modificar seus 
elementos.
*/

// Criando uma matriz bidimensional 3x3
// 'matrizBidimensional' é definida como um array de arrays, onde cada
// sub-array representa uma linha da matriz.
...
// Aqui, acessamos o elemento da segunda linha (índice 1, pois os
// índices começam em 0) e terceira coluna (índice 2).
// O acesso é feito diretamente usando os índices [1][2].
console.log(matrizBidimensional[1][2]); // Saída: 6

// Modificando um elemento (alterando o valor na terceira linha,
// primeira coluna para 10)
// Aqui, modificamos o valor na terceira linha (índice 2) e primeira
// coluna (índice 0).
// A modificação é feita atribuindo um novo valor ao elemento na
// posição especificada.
matrizBidimensional[2][0] = 10;

// Imprimindo a matriz modificada
// Após a modificação, imprimimos toda a matriz para visualizar as
// alterações.
...
matrizBidimensional.forEach((linha) => {
  // Cada linha é convertida em uma string utilizando o método 'join',
  // que concatena os elementos separados por um espaço.
  // Isso facilita a visualização da matriz inteira no console,
  // mostrando claramente a estrutura de linhas e colunas.
...
Uma matriz multidimensional (neste caso, uma matriz tridimensional) 
pode ser visualizada como uma sequência de matrizes bidimensionais.
Isso é útil para representar dados em três dimensões. Vamos criar uma 
matriz 2x3x3 (dois "blocos", cada um com três linhas e três colunas).
*/

// Criando uma matriz tridimensional 2x3x3
// 'matrizTridimensional' é uma matriz com duas "camadas" ou "blocos",
// cada uma contendo uma matriz 3x3.
...
// Acessando um elemento específico (no segundo bloco, terceira linha,
// segunda coluna)
// O elemento é acessado especificando o índice de cada dimensão:
// [bloco][linha][coluna].
console.log(matrizTridimensional[1][2][1]); // Saída: 17 - Isso acessa o segundo bloco, terceira linha, segunda coluna.

// Modificando um elemento (alterando o valor no primeiro bloco,
// primeira linha, terceira coluna para 20)
