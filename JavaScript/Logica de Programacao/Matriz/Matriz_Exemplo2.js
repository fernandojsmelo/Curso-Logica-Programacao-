...
function perguntarNome() {
  ...
  if (alunoIndex < numeroAlunos) {
    ...
    rl.question(`Digite o nome do aluno ${alunoIndex + 1}: `, function (nome) {
      ...
      // A variável 'notas' é local à função callback e só existe
      ...
      // Após inserir o aluno no array 'alunos' com seu nome e um array
      // de notas vazio, a função 'perguntarNotas' é chamada, passando
      // o array 'notas' como argumento.
      // Esta chamada inicia o processo de coleta das notas para o
      // aluno recém-adicionado.
      perguntarNotas(notas);
    });
  } else {
    // Se o índice do aluno atual é igual ou superior ao número total
    // de alunos, isso significa que todos os alunos já foram
    // processado s e seus nomes coletados.
    // Nesse caso, a função 'imprimirResultados' é chamada para
    // processar e exibir os resultados finais, como a média das notas e
    // a situação de aprovação ou reprovação de cada aluno.
    imprimirResultados();
  }
}
...
function perguntarNotas(notas) {
  ...
  // A condição verifica o tamanho do array 'notas'. Se menos de 3
  // notas foram inseridas, a função continua a pedir mais notas.
  if (notas.length < 3) {
    // Solicita ao usuário que insira a próxima nota. O número da nota
    // é determinado pelo tamanho atual do array 'notas',
    // acrescido de um, pois o índice do array começa em 0 mas a
    // contagem de notas para o usuário deve começar em 1.
    rl.question(`Digite a nota ${notas.length + 1}: `, function (nota) {
      // Adiciona a nota fornecida pelo usuário ao array 'notas'.
      // A função parseFloat converte a entrada do usuário (que é
      // recebida como uma string) para um número de ponto flutuante.
      // Isso é necessário porque as notas geralmente precisam ser
      // calculadas numericamente, por exemplo, para calcular médias.
      notas.push(parseFloat(nota));

      // Chama a função `perguntarNotas` recursivamente com o array
      // 'notas' atualizado.
      // Isso permite que o processo de inserção de notas continue até
      // que o total de 3 notas seja alcançado.
      perguntarNotas(notas);
    });
  } else {
    // Uma vez que 3 notas foram inseridas, o índice do aluno é
    // incrementado para mudar para o próximo aluno.
    alunoIndex++;

    // Chama a função `perguntarNome` para continuar o processo com o
    // próximo aluno.
    // Isso reinicia o ciclo de coleta de nomes e notas para um novo
    // aluno.
    perguntarNome();
  }
}
...
function imprimirResultados() {
  // Imprime um cabeçalho para indicar que os resultados serão listados
  // a seguir.
  console.log("\nResultados:");

  // Loop para processar cada aluno armazenado no array de alunos.
  // O array 'alunos' contém objetos, cada um representando um aluno
  // com seu nome e notas.
  ...
  // Encerra a interface de linha de comando após imprimir todos os
  // resultados.
  // Essa ação é importante para liberar o terminal e terminar a
  // execução do programa corretamente.
  rl.close();
}
