...
// Importando o módulo 'readline' do Node.js, que permite a interação
// com a linha de comando.
const readline = require("readline");

// Criando uma interface de leitura e escrita a partir do módulo
// 'readline'.
// 'process.stdin' é definido como entrada (input), permitindo que o
// programa leia dados do usuário via teclado.
// 'process.stdout' é definido como saída (output), permitindo que o
// programa escreva dados para o terminal.
...
// 'Map' é uma estrutura de dados que armazena pares de chave-valor,
// onde a chave é única.
let contatos = new Map();

// Função para adicionar um novo contato ao mapa 'contatos'.
// Recebe 'nome' e 'telefone' como parâmetros, que são as informações
// do contato.
function adicionarContato(nome, telefone) {
  // Adiciona o par nome-telefone ao mapa. O nome atua como chave, e o
  // telefone como valor.
...
// Define a função 'exibirContatos' que não recebe parâmetros e é
// usada para mostrar todos os contatos armazenados.
function exibirContatos() {
  // Exibe um cabeçalho no console para informar ao usuário que a
  // lista de contatos será mostrada a seguir.
  console.log("Lista de Contatos:");

  // Utiliza o método 'forEach' do objeto 'Map' para iterar sobre cada
  // par chave-valor no mapa 'contatos'.
  // 'forEach' é um método que permite executar uma função para cada
  // item no mapa.
  // A função passada para 'forEach' recebe dois argumentos:
  // 'telefone' (o valor) e 'nome' (a chave).
  // 'telefone' e 'nome' são, respectivamente, o número de telefone e
  // o nome do contato armazenados no mapa.
  contatos.forEach((telefone, nome) => {
    // Para cada par chave-valor, exibe o nome do contato seguido pelo
    // seu telefone.
    // A formatação `${nome}: ${telefone}` usa template literals para
    // inserir os valores de 'nome' e 'telefone' diretamente na string.
    console.log(`${nome}: ${telefone}`);
  });
}

// Define a função 'exibirMenu', que é responsável por apresentar o
// menu principal do programa ao usuário.
function exibirMenu() {
  // Exibe um conjunto de opções para o usuário, cada uma
  // correspondendo a uma ação diferente no gerenciador de contatos.
  console.log("\nEscolha uma opção:");
  ...
  // 'rl.question' exibe uma mensagem e aguarda a entrada do usuário,
  // que é processada em uma função callback.
  rl.question("Opção: ", (opcao) => {
    // Utiliza uma estrutura de controle 'switch' para dirigir o fluxo
    // do programa baseado na entrada do usuário.
    switch (opcao) {
      case "1": // Caso a opção 1 seja selecionada:
        ...
        rl.question("Nome do Contato: ", (nome) => {
          ...
          rl.question("Telefone: ", (telefone) => {
            // Chama a função 'adicionarContato' com os dados
      ...
      case "2": // Caso a opção 2 seja selecionada:
        ...
        rl.question("Nome do Contato: ", (nome) => {
          // Chama a função 'removerContato' para excluir o contato
          // especificado.
          removerContato(nome);
      ...
      case "4": // Caso a opção 4 seja selecionada:
        ...
        // Fecha a interface de linha de comando, efetivamente
