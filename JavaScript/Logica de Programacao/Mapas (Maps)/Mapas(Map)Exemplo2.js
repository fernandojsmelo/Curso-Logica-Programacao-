...
// Inicializa um novo objeto Map para armazenar pares de filmes e suas
// avaliações.
...
function adicionarFilme(nome, avaliacao) {
  // Usa o método 'set' para adicionar ou atualizar o nome do filme
  // com a avaliação correspondente no mapa.
  filmesAvaliacoes.set(nome, avaliacao);

  // Exibe uma mensagem no console confirmando a adição do filme com
  // sua avaliação.
...
function excluirFilme(nome) {
  ...
  if (filmesAvaliacoes.has(nome)) {
    // Se o filme existe, usa o método 'delete' para remover o filme
    // do mapa.
    ...
    // Se o filme não for encontrado, exibe uma mensagem no console
    // informando que o filme não foi encontrado.
...
function exibirListaFilmes() {
  ...
  // Itera sobre cada entrada no mapa de filmes usando o método
  // 'forEach'.
...
function verificarFilme(nome) {
  ...
  if (filmesAvaliacoes.has(nome)) {
    // Se o filme estiver na lista, exibe uma mensagem no console
    // confirmando sua presença.
    console.log(`O filme "${nome}" está na lista.`);
  } else {
    // Se o filme não estiver na lista, exibe uma mensagem no console
    // informando sua ausência.
    console.log(`O filme "${nome}" não está na lista.`);
  }
}

// Chama a função 'adicionarFilme' para adicionar alguns filmes e suas
// avaliações ao mapa.
adicionarFilme("Matrix", 9.0);
adicionarFilme("Senhor dos Anéis", 8.5);
adicionarFilme("Harry Potter", 8.0);

// Chama a função 'exibirListaFilmes' para exibir todos os filmes e suas
// avaliações.
exibirListaFilmes();

// Chama a função 'verificarFilme' para verificar se certos filmes estão
// na lista.
verificarFilme("Matrix");
verificarFilme("Batman");

// Chama a função 'excluirFilme' para remover um filme específico da
// lista.
excluirFilme("Senhor dos Anéis");

// Chama a função 'exibirListaFilmes' novamente para mostrar a lista
// de filmes após a exclusão de um filme.
exibirListaFilmes();
