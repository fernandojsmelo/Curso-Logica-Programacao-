...
pode percorrer os pares chave-valor usando loops, como for...of ou 
forEach.
...
// Utiliza o método 'has' do objeto 'Map' para verificar se a chave
// (nome do aluno) 'Pedro' existe no mapa 'notasDosAlunos'.
// O método 'has' retorna um valor booleano: 'true' se a chave existir
// no mapa e 'false' caso contrário.
if (notasDosAlunos.has(aluno)) {
  // Se o método 'has' retornar 'true', significa que o aluno 'Pedro'
  // está registrado no mapa.
  // Imprime uma mensagem no console confirmando a presença do aluno.
  console.log(`\n${aluno} está no mapa de notas.`);
} else {
  // Se o método 'has' retornar 'false', significa que o aluno 'Pedro'
  // não está registrado no mapa.
  // Imprime uma mensagem no console indicando que o aluno não está
  // presente.
  console.log(`\n${aluno} não está no mapa de notas.`);
}

// Removendo um aluno, João, do mapa.
// O método 'delete' é usado para remover um par chave-valor baseado
// na chave.
notasDosAlunos.delete("João");

// Imprime uma mensagem de cabeçalho no console para indicar que as
// notas dos alunos serão listadas após a remoção de 'João'.
console.log("\nNotas dos Alunos (após remoção de João):");

// Utiliza o método 'forEach' do objeto 'Map' para iterar sobre cada
// par chave-valor restante no mapa.
// Para cada par, o método 'forEach' executa a função callback fornecida,
// que recebe a nota (valor) e o nome do aluno (chave).
notasDosAlunos.forEach((nota, aluno) => {
  // Imprime no console o nome do aluno e a nota correspondente.
  // Essa listagem confirmará visualmente se 'João' foi removido, pois
  // seu nome e nota não
