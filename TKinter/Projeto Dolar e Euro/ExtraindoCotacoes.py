...
moedaSelecionada = ttk.Combobox(janela, font=("Arial 30"))
...
def pesquisarItem():
    ...
    # --headless vai ser descontinuada nas próximas versões
    options.add_argument("--headless")

    # Aguarda 1 segundo para dar tempo do computador processar as informações
    tempoPausaComputador.sleep(1)

    #Instanciando da classe Options para configurar o headless
    opcoes = Options()

    #True - Oculta o navegador do Chrome
    #False - Reexibe o navegador do Chrome
    opcoes.headless = True

    meuNavegador = webdriver.Chrome(executable_path=r"/usr/local/bin/chrome-linux64\chrome.exe", options=opcoes)
    ...
    valorMoeda.config(text= "\n\n" + str(moedaSelecionada.get()) + ":" + valorDolarPeloGoogle + "\n\n")
